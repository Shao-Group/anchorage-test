...
results_dir="results-solo-megahit"
...
out_postfix="solo"
megahit_prefix=$results_dir/"megahit_alln"_$out_postfix
if [ ! -d "$megahit_prefix" ]; then
        mkdir $megahit_prefix
else
        rm -r $megahit_prefix
        mkdir $megahit_prefix
fi
...
megahit_prefix=$results_dir/"megahit_500n"_$out_postfix
if [ ! -d "$megahit_prefix" ]; then
        mkdir $megahit_prefix
else
        rm -r $megahit_prefix
...
dir_intm="data-sim"
results_dir="results-sim-megahit"
...
results_dir="results-simthru-megahit"
...
results_dir="results-simrepeat-megahit"
