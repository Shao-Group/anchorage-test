...
def plot_4(df1, df2, image_name):
    ...
    if quast_dir.endswith('sim'):
        axes[1, 1].legend(bbox_to_anchor=(0.4, 1.0), title="depth", loc = 'best', fontsize=13)
    else:
        axes[1, 0].legend(bbox_to_anchor=(1, 1.0), title="depth", loc = 'best', fontsize=13)
...
def assembly_to_algo(assemblies):
    ...
    if assemblies.startswith('anchorage_'):
        ...
        algo = 'SPAdes500'
    elif assemblies.startswith('spades_alln_'):
        algo = 'SPAdes'
...
if __name__ == "__main__":
    ...
    # sim without artifacts
    quast_dir = argv[1]
    solo_df_fillna = read_dir_to_pd(quast_dir, to_fillna=True)
    solo_df_keepna = read_dir_to_pd(quast_dir, to_fillna=False)
    plot_4(solo_df_fillna, solo_df_keepna, "plot/sim.pdf")

    # read throughs
    quast_dir2 = argv[2]
    solo_df_fillna = read_dir_to_pd(quast_dir2, to_fillna=True)
    solo_df_keepna = read_dir_to_pd(quast_dir2, to_fillna=False)
    plot_4(solo_df_fillna, solo_df_keepna, "plot/sim-thru.pdf".format(quast_dir2))