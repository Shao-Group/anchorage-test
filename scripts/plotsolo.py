...
def plot_solo(df, y_name, code, ax):
    # plot swarmplot
    sns.swarmplot(x="method", y=y_name, data=df, zorder=1, ax=ax, order=order)
    sns.barplot(x='method', y=y_name, data=df, color='red', ax=ax, alpha=0.5, zorder=0, errorbar = None, order=order,
    ...
    if(code % 2 == 1):
        ...
        for i in ax.containers:
            ax.bar_label(i,label_type='edge', rotation=90, fmt=lambda x: "{v:.2f}".format(v=x) if x > 0 else '', fontsize=15)
    ax.set_xlabel("")
...
def assembly_to_algo(assemblies):
    ...
    if assemblies.startswith('anchorage_'):
        algo = 'Anchorage'
    elif assemblies.startswith('spades_1iter_500') or assemblies.startswith('spades_500n_1'):
        algo = 'SPAdes500'
...
if __name__ == "__main__":
    ...
    # read df
    solo_df_fillna = read_dir_to_pd(quast_res_dir, to_fillna=True)
    solo_df_keepna = read_dir_to_pd(quast_res_dir, to_fillna=False)

    # plot solo results
    plot_4(solo_df_fillna, solo_df_keepna, "plot/solo.pdf")
